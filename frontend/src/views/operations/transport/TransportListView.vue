<template>
  <div class="tsp-list">
    <TransportCreateForm v-if="createOpen" @close="createOpen = false" @created="onCreated" />
      ...
      <template v-else-if="!showDeleted">
        ...
        <div v-else class="order-grid">
          <div v-for="order in store.orders" :key="order.id"
            ...
            <div class="card-carrier">{{ order.carrier?.name }}</div>
            <div class="card-footer">
              <span class="card-qty">{{ Number(order.quantity) }} {{ t('truckUnit') }}</span>
              <span class="card-amount accent">฿{{ Number(order.amount).toLocaleString() }}</span>
            </div>
          </div>
        </div>
      </template>
      ...
      <template v-else>
        ...
        <div v-else class="order-grid">
          <div v-for="order in store.deletedOrders" :key="order.id" class="order-card deleted-card">
            ...
            <div class="card-date">{{ formatDate(order.orderDate) }}</div>
            <div class="card-carrier muted">{{ order.carrier?.name }}</div>
...
import { hardDeleteTransportOrder } from '../../../api/transport'
import TransportCreateForm from './TransportCreateForm.vue'
import TransportDetailDrawer from './TransportDetailDrawer.vue'
...
async function executeHardDelete() {
  try {
    await hardDeleteTransportOrder(hardDeleteDialog.value.orderId)
...
.card-date { font-size: 12px; color: var(--color-text-muted); margin-bottom: 4px; }
.card-carrier { font-size: 13px; font-weight: 600; margin-bottom: 10px; }
...
.card-qty { font-size: 12px; color: var(--color-text-muted); }
