<template>
  <div class="inq-view">
    ...
    <template v-else-if="store.receivingVolumeResult">
      <div class="summary-card">
        <div class="summary-item">
          <span class="summary-label">{{ t('totalQuantity') }}</span>
          <span class="summary-value">{{ store.receivingVolumeResult.summary.totalQuantity.toFixed(1) }}</span>
        </div>
        <div class="summary-item">
          <span class="summary-label">{{ t('totalAmount') }}</span>
          <span class="summary-value accent">฿{{ store.receivingVolumeResult.summary.totalAmount.toLocaleString() }}</span>
        </div>
        <div class="summary-item">
          <span class="summary-label">{{ t('avgUnitPrice') }}</span>
          <span class="summary-value">฿{{ store.receivingVolumeResult.summary.avgUnitPrice.toFixed(2) }}</span>
        </div>
      </div>

      <div v-if="store.receivingVolumeResult.rows.length === 0" class="empty-state">{{ t('noResultData') }}</div>

      <div v-else class="result-table">
        <div class="row head">
          <span>{{ t('colDate') }}</span><span>{{ t('colOrderId') }}</span><span>{{ t('totalQty') }}</span><span>{{ t('colAvgPrice') }}</span><span>{{ t('colAmount') }}</span>
        </div>
        <div v-for="(r, i) in store.receivingVolumeResult.rows" :key="i" class="row">
...
function resetFilters() {
  ...
  store.receivingVolumeResult = null
}

function buildPdfOptions() {
  if (!store.receivingVolumeResult) return null
  return {
    title: 'รายงานปริมาณการรับสินค้า',
    subtitle: `ช่วงวันที่ ${startDate.value || 'ไม่จำกัด'} ~ ${endDate.value || 'ไม่จำกัด'}`,
    columns: [
      { header: 'วันที่', key: 'date' },
      { header: 'เลขใบรับ', key: 'orderId' },
      { header: 'น้ำหนัก', key: 'quantity', align: 'right' as const },
      { header: 'ราคาเฉลี่ย', key: 'avgPrice', align: 'right' as const },
    ...
    rows: store.receivingVolumeResult.rows.map((r) => ({
      ...
      avgPrice: r.avgPrice.toFixed(2),
      amount: r.amount.toLocaleString(),
    })),
    summary: [
      { label: 'จำนวนรวม', value: store.receivingVolumeResult.summary.totalQuantity.toFixed(1) },
      { label: 'ยอดเงินรวม', value: `฿${store.receivingVolumeResult.summary.totalAmount.toLocaleString()}` },
      { label: 'ราคาเฉลี่ย', value: `฿${store.receivingVolumeResult.summary.avgUnitPrice.toFixed(2)}` },
    ],
    fileName: `receiving-volume-inquiry_${new Date().toISOString().slice(0, 10)}.pdf`,
  }
}
...
async function sharePdf() {
  ...
  try {
    ...
    const result = await sharePdfBlob(blob, opts.fileName, '進貨量查詢')
...
.row { display: grid; grid-template-columns: 1fr 1.4fr 0.8fr 0.9fr 1fr; gap: 8px; padding: 10px 14px; font-size: 13px; border-bottom: 1px solid var(--color-border); }
