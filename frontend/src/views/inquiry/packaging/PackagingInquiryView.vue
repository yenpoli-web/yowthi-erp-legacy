<template>
  <div class="inq-view">
    ...
    <div class="filter-card">
      ...
      <div class="filter-row">
        <div class="field">
          <label class="field-label">{{ t('packagingItemMultiSelect') }}</label>
          <button class="select-btn" @click="itemModalOpen = true">
            {{ selectedItemNames || t('allPackagingItems') }}
          </button>
        </div>
      </div>
    ...
    <template v-else-if="store.packagingResult">
      <div class="summary-card">
        <div class="summary-item">
          <span class="summary-label">{{ t('packagingQty') }}</span>
          <span class="summary-value">{{ store.packagingResult.summary.totalQuantity.toFixed(1) }}</span>
        </div>
        <div class="summary-item">
          <span class="summary-label">{{ t('totalAmount') }}</span>
          <span class="summary-value accent">฿{{ store.packagingResult.summary.totalAmount.toLocaleString() }}</span>
        </div>
      </div>

      <div v-if="store.packagingResult.rows.length === 0" class="empty-state">{{ t('noResultData') }}</div>

      <div v-else class="result-table">
        <div class="row head">
          <span>{{ t('colDate') }}</span><span>{{ t('colEmployeeId') }}</span><span>{{ t('colEmployeeName') }}</span><span>{{ t('colPackagingItem') }}</span><span>{{ t('packagingQty') }}</span><span>{{ t('colWageRate') }}</span><span>{{ t('colAmount') }}</span>
        </div>
        <div v-for="(r, i) in store.packagingResult.rows" :key="i" class="row">
    ...
    <div v-if="itemModalOpen" class="modal-backdrop" @click.self="itemModalOpen = false">
      <div class="modal-panel">
        <div class="modal-header">
          <span class="modal-title">{{ t('selectPackagingItemTitle') }}</span>
          <button class="close-btn" @click="itemModalOpen = false">✕</button>
        </div>
        <div class="item-grid">
          <button
            v-for="i in packagingItemsStore.packagingItems"
            :key="i.id"
            class="item-card"
            :class="{ selected: selectedItemIds.includes(i.id) }"
            @click="toggleItem(i.id)"
          >
            <i v-if="selectedItemIds.includes(i.id)" class="pi pi-check check-icon"></i>
            <span class="item-name">{{ i.name }}</span>
            <span class="item-id">{{ i.id }}</span>
          </button>
        </div>
        <div class="modal-footer">
          <button class="btn-secondary" @click="selectedItemIds = []">{{ t('clear') }}</button>
          <button class="btn-primary" @click="itemModalOpen = false">{{ t('confirm') }}（{{ selectedItemIds.length }}）</button>
        </div>
      </div>
    </div>
  </div>
...
function search() {
  store.searchPackaging({
    startDate: startDate.value || undefined,
    endDate: endDate.value || undefined,
    packagingItemIds: selectedItemIds.value.length ? selectedItemIds.value.join(',') : undefined,
  })
}
...
function resetFilters() {
  ...
  store.packagingResult = null
}

function buildPdfOptions() {
  if (!store.packagingResult) return null
  return {
    title: 'รายงานการบรรจุภัณฑ์',
    ...
    columns: [
      ...
      { header: 'รายการบรรจุภัณฑ์', key: 'itemName' },
      { header: 'จำนวน (ชิ้น)', key: 'quantity', align: 'right' as const },
    ...
    summary: [
      { label: 'จำนวนรวม', value: store.packagingResult.summary.totalQuantity.toFixed(1) },
      { label: 'ยอดเงินรวม', value: `฿${store.packagingResult.summary.totalAmount.toLocaleString()}` },
    ],
    fileName: `packaging-inquiry_${new Date().toISOString().slice(0, 10)}.pdf`,
  }
}
...
async function sharePdf() {
  ...
  try {
    ...
    const result = await sharePdfBlob(blob, opts.fileName, '包裝查詢')
...
onMounted(() => {
  packagingItemsStore.fetchPackagingItems()
})
</script>
...
.row { display: grid; grid-template-columns: 0.8fr 0.7fr 1fr 1fr 0.8fr 0.8fr 0.9fr; gap: 8px; padding: 10px 14px; font-size: 13px; border-bottom: 1px solid var(--color-border); min-width: 760px; }
