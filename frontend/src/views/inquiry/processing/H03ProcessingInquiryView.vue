<template>
  <div class="inq-view">
    ...
    <template v-else-if="store.h03ProcessingResult">
      <div class="summary-card">
        <div class="summary-item">
          <span class="summary-label">{{ t('colDefectQty') }}</span>
          <span class="summary-value">{{ store.h03ProcessingResult.summary.totalH01Defect.toFixed(1) }}</span>
        </div>
        <div class="summary-item">
          <span class="summary-label">{{ t('colH03Output') }}</span>
          <span class="summary-value">{{ store.h03ProcessingResult.summary.totalH03Output.toFixed(1) }}</span>
        </div>
        <div class="summary-item">
          <span class="summary-label">{{ t('totalAmount') }}</span>
          <span class="summary-value accent">฿{{ store.h03ProcessingResult.summary.totalAmount.toLocaleString() }}</span>
        </div>
      </div>

      <div v-if="store.h03ProcessingResult.rows.length === 0" class="empty-state">{{ t('noResultData') }}</div>

      <div v-else class="result-table">
        <div class="row head">
          <span>{{ t('colDate') }}</span><span>{{ t('colItem') }}</span><span>{{ t('colDefectQty') }}</span><span>{{ t('colH03Output') }}</span><span>{{ t('colAmount') }}</span>
        </div>
        <div v-for="(r, i) in store.h03ProcessingResult.rows" :key="i" class="row">
          <span>{{ formatDate(r.date) }}</span>
          <span>{{ r.receivingItemName }}</span>
          <span>{{ r.h01Defect.toFixed(1) }}</span>
          <span>{{ r.h03Output.toFixed(1) }}</span>
...
function resetFilters() {
  ...
  store.h03ProcessingResult = null
}

function buildPdfOptions() {
  if (!store.h03ProcessingResult) return null
  return {
    title: 'รายงานการแปรรูป H03',
    subtitle: `ช่วงวันที่ ${startDate.value || 'ไม่จำกัด'} ~ ${endDate.value || 'ไม่จำกัด'}`,
    columns: [
      { header: 'วันที่', key: 'date' },
      { header: 'สินค้า', key: 'itemName' },
      { header: 'ไม่ได้มาตรฐาน', key: 'h01Defect', align: 'right' as const },
      { header: 'สำเร็จ (H03)', key: 'h03Output', align: 'right' as const },
    ...
    rows: store.h03ProcessingResult.rows.map((r) => ({
      ...
      h01Defect: r.h01Defect.toFixed(1),
      h03Output: r.h03Output.toFixed(1),
      amount: r.amount.toLocaleString(),
    })),
    summary: [
      { label: 'ไม่ได้มาตรฐานรวม', value: store.h03ProcessingResult.summary.totalH01Defect.toFixed(1) },
      { label: 'สำเร็จรวม (H03)', value: store.h03ProcessingResult.summary.totalH03Output.toFixed(1) },
      { label: 'ยอดเงินรวม', value: `฿${store.h03ProcessingResult.summary.totalAmount.toLocaleString()}` },
    ],
    fileName: `h03-processing-inquiry_${new Date().toISOString().slice(0, 10)}.pdf`,
  }
}
...
async function sharePdf() {
  ...
  try {
    ...
    const result = await sharePdfBlob(blob, opts.fileName, 'H03 完成品加工查詢')
...
.row { display: grid; grid-template-columns: 0.8fr 1fr 0.8fr 0.8fr 0.8fr; gap: 8px; padding: 10px 14px; font-size: 13px; border-bottom: 1px solid var(--color-border); }
