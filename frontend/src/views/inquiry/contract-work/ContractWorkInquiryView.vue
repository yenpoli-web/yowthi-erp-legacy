<template>
  <div class="inq-view">
    ...
    <div class="filter-card">
      ...
      <div class="filter-row">
        <div class="field">
          <label class="field-label">{{ t('supplierMultiSelect') }}</label>
          <button class="select-btn" @click="supplierModalOpen = true">
            {{ selectedSupplierNames || t('allSuppliers') }}
          </button>
        </div>
        <div class="field">
          <label class="field-label">{{ t('productMultiSelect') }}</label>
          <button class="select-btn" @click="productModalOpen = true">
            {{ selectedProductNames || t('allProducts') }}
          </button>
        </div>
      </div>
    ...
    <template v-else-if="store.contractWorkResult">
      <div class="summary-card">
        <div class="summary-item">
          <span class="summary-label">{{ t('totalQtyLabel') }}</span>
          <span class="summary-value">{{ store.contractWorkResult.summary.totalQuantity.toFixed(1) }}</span>
        </div>
        <div class="summary-item">
          <span class="summary-label">{{ t('totalWeight') }}</span>
          <span class="summary-value">{{ store.contractWorkResult.summary.totalWeight.toFixed(1) }}</span>
        </div>
        <div class="summary-item">
          <span class="summary-label">{{ t('totalAmount') }}</span>
          <span class="summary-value accent">฿{{ store.contractWorkResult.summary.totalAmount.toLocaleString() }}</span>
        </div>
      </div>

      <div v-if="store.contractWorkResult.rows.length === 0" class="empty-state">{{ t('noResultData') }}</div>

      <div v-else class="result-table">
        <div class="row head">
          <span>{{ t('colDate') }}</span><span>{{ t('colSupplierId') }}</span><span>{{ t('colSupplierName') }}</span><span>{{ t('colProduct') }}</span><span>{{ t('colQty') }}</span><span>{{ t('weight') }}</span><span>{{ t('colUnitPrice') }}</span><span>{{ t('colAmount') }}</span>
        </div>
        <div v-for="(r, i) in store.contractWorkResult.rows" :key="i" class="row">
          <span>{{ formatDate(r.date) }}</span>
          <span>{{ r.supplierId }}</span>
    ...
    <!-- 廠商多選 Modal -->
    <div v-if="supplierModalOpen" class="modal-backdrop" @click.self="supplierModalOpen = false">
      <div class="modal-panel">
        <div class="modal-header">
          <span class="modal-title">{{ t('selectSupplierTitle') }}</span>
          <button class="close-btn" @click="supplierModalOpen = false">✕</button>
        </div>
        <div class="item-grid">
          <button
            ...
            :key="s.id"
            class="item-card"
            :class="{ selected: selectedSupplierIds.includes(s.id) }"
            @click="toggleSupplier(s.id)"
          >
            <i v-if="selectedSupplierIds.includes(s.id)" class="pi pi-check check-icon"></i>
            <span class="item-name">{{ s.name }}</span>
            <span class="item-id">{{ s.id }}</span>
          </button>
        </div>
        <div class="modal-footer">
          <button class="btn-secondary" @click="selectedSupplierIds = []">{{ t('clear') }}</button>
          <button class="btn-primary" @click="supplierModalOpen = false">{{ t('confirm') }}（{{ selectedSupplierIds.length }}）</button>
        </div>
      </div>
    </div>

    <!-- 商品多選 Modal -->
    <div v-if="productModalOpen" class="modal-backdrop" @click.self="productModalOpen = false">
      <div class="modal-panel">
        <div class="modal-header">
          <span class="modal-title">{{ t('selectProductTitle') }}</span>
          <button class="close-btn" @click="productModalOpen = false">✕</button>
        </div>
        <div class="item-grid">
          <button
          ...
          >
            <i v-if="selectedProductIds.includes(p.id)" class="pi pi-check check-icon"></i>
            <span class="item-name">{{ p.name }}</span>
            <span class="item-id">{{ p.id }}</span>
          </button>
        </div>
        <div class="modal-footer">
          <button class="btn-secondary" @click="selectedProductIds = []">{{ t('clear') }}</button>
          <button class="btn-primary" @click="productModalOpen = false">{{ t('confirm') }}（{{ selectedProductIds.length }}）</button>
        </div>
      </div>
    </div>
  </div>
...
const selectedSupplierNames = computed(() =>
  suppliersStore.suppliers
    .filter((s) => selectedSupplierIds.value.includes(s.id))
    .map((s) => s.name)
    .join('、'),
)
...
function toggleSupplier(id: string) {
  const idx = selectedSupplierIds.value.indexOf(id)
  if (idx === -1) selectedSupplierIds.value.push(id)
  else selectedSupplierIds.value.splice(idx, 1)
}
...
function search() {
  store.searchContractWork({
    startDate: startDate.value || undefined,
    endDate: endDate.value || undefined,
    supplierIds: selectedSupplierIds.value.length ? selectedSupplierIds.value.join(',') : undefined,
    productIds: selectedProductIds.value.length ? selectedProductIds.value.join(',') : undefined,
  })
}
...
function resetFilters() {
  ...
  store.contractWorkResult = null
}

function buildPdfOptions() {
  if (!store.contractWorkResult) return null
  return {
    title: 'รายงานการจ้างผลิตภายนอก',
    subtitle: `ช่วงวันที่ ${startDate.value || 'ไม่จำกัด'} ~ ${endDate.value || 'ไม่จำกัด'}`,
    columns: [
      { header: 'วันที่', key: 'date' },
      { header: 'รหัสซัพพลายเออร์', key: 'supplierId' },
      { header: 'ชื่อซัพพลายเออร์', key: 'supplierName' },
    ...
    summary: [
      { label: 'จำนวนรวม', value: store.contractWorkResult.summary.totalQuantity.toFixed(1) },
      { label: 'น้ำหนักรวม', value: store.contractWorkResult.summary.totalWeight.toFixed(1) },
      { label: 'ยอดเงินรวม', value: `฿${store.contractWorkResult.summary.totalAmount.toLocaleString()}` },
    ],
    fileName: `contract-work-inquiry_${new Date().toISOString().slice(0, 10)}.pdf`,
  }
}
...
async function sharePdf() {
  ...
  try {
    ...
    const result = await sharePdfBlob(blob, opts.fileName, '代工查詢')
...
.row { display: grid; grid-template-columns: 0.7fr 0.7fr 0.9fr 0.9fr 0.6fr 0.6fr 0.7fr 0.8fr; gap: 8px; padding: 10px 14px; font-size: 13px; border-bottom: 1px solid var(--color-border); min-width: 860px; }
