...
async function saveAdd() {
  ...
  saving.value = true
  try {
    await farmersStore.createFarmer({
      id:          addForm.id.trim(),
...
function startEdit() {
  ...
  editForm.address     = f.address     ?? ''
  editForm.bankName    = f.bankName    ?? ''
  editForm.bankAccount = f.bankAccount ?? ''
  editForm.imageUrl    = f.imageUrl    ?? undefined
...
async function saveEdit() {
  if (!selectedFarmer.value || !editForm.name.trim()) return
  saving.value = true
  try {
    const updated = await farmersStore.updateFarmer(selectedFarmer.value.id, {
...
async function doSoftDelete() {
  if (!selectedFarmer.value) return
  try {
    await farmersStore.softDeleteFarmer(selectedFarmer.value.id)
...
async function doHardDelete() {
  if (!selectedFarmer.value) return
  try {
    await farmersStore.hardDeleteFarmer(selectedFarmer.value.id)
...
async function toggleShowDeleted() {
  ...
  if (showDeleted.value) {
    await farmersStore.fetchDeletedFarmers()
  }
}

async function doHardDeleteFromList(id: string) {
  try {
    await farmersStore.hardDeleteFarmer(id)
    await farmersStore.fetchDeletedFarmers()
    toastStore.showToast(L('ลบถาวรแล้ว', '已永久刪除'), 'success')
  } catch (e: any) {
    toastStore.showToast((e as any)?.response?.data?.message || L('ลบล้มเหลว', '刪除失敗'), 'danger')
...
onMounted(() => farmersStore.fetchFarmers())
</script>
  ...
  <div class="farmer-layout">
      ...
      <div v-if="showAddForm" class="add-wrap">
        <form class="add-form" @submit.prevent="saveAdd">
          <h2 class="form-title">{{ L('เพิ่มเกษตรกร', '新增農民') }}</h2>

          <div class="form-group">
            <label class="form-label">{{ L('รหัสเกษตรกร', '農民 ID') }} <span class="required">*</span></label>
            <input v-model="addForm.id" class="form-input" :placeholder="L('ใส่รหัสเกษตรกร', '輸入農民 ID')" required />
          </div>
          <div class="form-group">
      ...
      <div v-else class="farmer-main">
        ...
        <div class="list-panel">
          ...
          <div class="list-header">
            <span class="list-count">
              {{ showDeleted ? farmersStore.deletedFarmers.length : farmersStore.farmers.length }} {{ L('รายการ', '筆') }}
            </span>
            <div class="header-actions">
              ...
              <button class="btn-add" @click="openAddForm">
                ...
                <span class="btn-add-text">{{ L('เพิ่มเกษตรกร', '新增農民') }}</span>
              </button>
            </div>
          </div>
          ...
          <template v-if="!showDeleted">
            <div v-if="farmersStore.loading" class="list-state">
              <i class="pi pi-spin pi-spinner"></i>
            </div>
            <div v-else-if="farmersStore.farmers.length === 0" class="list-state">
              <i class="pi pi-users"></i>
              <p>{{ L('ไม่มีข้อมูลเกษตรกร', '尚無農民資料') }}</p>
            </div>
            <div v-else class="farmer-grid">
              <button
                v-for="farmer in farmersStore.farmers"
                :key="farmer.id"
                class="farmer-card"
                :class="{ active: selectedFarmer?.id === farmer.id }"
                @click="selectFarmer(farmer)"
              >
                <span class="card-avatar" :style="farmer.imageUrl ? {} : { background: avatarColor(farmer.id) }">
                  <img v-if="farmer.imageUrl" class="card-avatar--img" :src="resolveImageUrl(farmer.imageUrl)" :alt="farmer.name" />
                  <template v-else>{{ initials(farmer.name) }}</template>
                </span>
                <div class="card-body">
                  <span class="card-id-badge">{{ farmer.id }}</span>
                  <p class="card-name">{{ farmer.name }}</p>
                  <p v-if="farmer.phone" class="card-phone">{{ farmer.phone }}</p>
                </div>
              </button>
            </div>
          </template>
          ...
          <template v-else>
            ...
            <div v-else-if="farmersStore.deletedFarmers.length === 0" class="list-state">
              <i class="pi pi-trash"></i>
              <p>{{ L('ไม่มีรายการที่ลบแล้ว', '沒有已刪除的記錄') }}</p>
            </div>
            <div v-else class="farmer-grid">
              <div
                v-for="farmer in farmersStore.deletedFarmers"
                :key="farmer.id"
                class="farmer-card deleted"
              >
                <span class="card-avatar" :style="farmer.imageUrl ? { filter: 'grayscale(1)' } : { background: avatarColor(farmer.id), filter: 'grayscale(1)' }">
                  <img v-if="farmer.imageUrl" class="card-avatar--img" :src="resolveImageUrl(farmer.imageUrl)" :alt="farmer.name" />
                  <template v-else>{{ initials(farmer.name) }}</template>
                </span>
                <div class="card-body">
                  ...
                  <p class="card-name">{{ farmer.name }}</p>
                  <p v-if="farmer.phone" class="card-phone">{{ farmer.phone }}</p>
                  <p class="deleted-meta">🗑 {{ (farmer as any).deletedAt ? new Date((farmer as any).deletedAt).toLocaleDateString() : '' }}</p>
                  <button
                    v-if="authStore.role === 'ADMIN'"
                    class="btn btn-danger-solid btn-sm"
                    @click="doHardDeleteFromList(farmer.id)"
        ...
        <!-- Detail Drawer -->
        <div v-if="selectedFarmer" class="detail-drawer" :class="{ open: !!selectedFarmer }">
          <div class="drawer-header">
            <span class="drawer-avatar" :style="selectedFarmer.imageUrl ? {} : { background: avatarColor(selectedFarmer.id) }">
              <img v-if="selectedFarmer.imageUrl" class="drawer-avatar--img" :src="resolveImageUrl(selectedFarmer.imageUrl)" :alt="selectedFarmer.name" />
              <template v-else>{{ initials(selectedFarmer.name) }}</template>
            </span>
            <div class="drawer-title-area">
              <span class="drawer-name">{{ selectedFarmer.name }}</span>
              <span class="drawer-id-badge">{{ selectedFarmer.id }}</span>
            </div>
          ...
          <div class="drawer-body">
            <template v-if="!isEditMode">
              <div v-if="selectedFarmer.imageUrl" class="field-image-wrap">
                <img class="field-image" :src="selectedFarmer.imageUrl" :alt="selectedFarmer.name" />
              </div>
              <div class="field-row">
                <span class="field-label">{{ L('โทรศัพท์', '電話') }}</span>
                <span class="field-value">{{ selectedFarmer.phone || '—' }}</span>
              </div>
              <div class="field-row">
                <span class="field-label">{{ L('ที่อยู่', '住址') }}</span>
                <span class="field-value">{{ selectedFarmer.address || '—' }}</span>
              </div>
              <div class="field-row">
                <span class="field-label">{{ L('ชื่อธนาคาร', '銀行名稱') }}</span>
                <span class="field-value">{{ selectedFarmer.bankName || '—' }}</span>
              </div>
              <div class="field-row">
                <span class="field-label">{{ L('เลขบัญชี', '銀行帳號') }}</span>
                <span class="field-value">{{ selectedFarmer.bankAccount || '—' }}</span>
              </div>
            </template>
            <template v-else>
              <div class="form-group">
                <label class="form-label">{{ L('รหัสเกษตรกร', '農民 ID') }}</label>
                <input :value="selectedFarmer.id" class="form-input" disabled />
              </div>
              <div class="form-group">
                <label class="form-label">{{ L('ชื่อ', '姓名') }} <span class="required">*</span></label>
    ...
    <ConfirmDialog
      ...
      :message="L(`ต้องการลบเกษตรกร「${selectedFarmer?.name}」ใช่ไหม?`, `確定要刪除農民「${selectedFarmer?.name}」嗎？`)"
    ...
    <ConfirmDialog
      ...
      :title="L('ลบถาวร', '永久刪除')"
      :message="L(`การดำเนินการนี้จะลบเกษตรกร「${selectedFarmer?.name}」อย่างถาวรและไม่สามารถกู้คืนได้`, `此操作將永久刪除農民「${selectedFarmer?.name}」，且無法復原。確定繼續嗎？`)"
...
<style scoped>
.farmer-layout { height: 100%; display: flex; flex-direction: column; overflow: hidden; }
...
.farmer-main { flex: 1; display: flex; overflow: hidden; position: relative; }
...
.farmer-grid { display: grid; grid-template-columns: 1fr; gap: 8px; }
@media (min-width: 540px) { .farmer-grid { grid-template-columns: repeat(2, 1fr); } }
@media (min-width: 1280px) { .farmer-grid { grid-template-columns: repeat(3, 1fr); } }
.farmer-card { display: flex; align-items: flex-start; gap: 12px; padding: 12px; background: var(--color-card); border: 1px solid var(--color-border); border-radius: 10px; color: var(--color-text); text-align: left; width: 100%; transition: background 0.15s, border-color 0.15s; }
.farmer-card:hover { background: var(--color-card-hover); border-color: var(--color-text-dim); }
.farmer-card.active { border-color: var(--color-accent); background: var(--color-card-hover); }
.farmer-card.deleted { opacity: 0.65; border-style: dashed; border-color: var(--color-danger); cursor: default; }
...
.field-image-wrap { padding: 12px 0; border-bottom: 1px solid var(--color-border); }
.field-image { width: 100%; max-height: 200px; object-fit: contain; border-radius: 8px; background: var(--color-card); }
