...
const addForm = reactive({ id: '', name: '', unitWeight: undefined as number | undefined, productType: 'EXPORT' as 'EXPORT' | 'DOMESTIC', imageUrl: undefined as string | undefined })

function resetAddForm() { addForm.id = ''; addForm.name = ''; addForm.unitWeight = undefined; addForm.productType = 'EXPORT'; addForm.imageUrl = undefined }
...
async function saveAdd() {
  ...
  try {
    await productsStore.createProduct({
      ...
      unitWeight:  addForm.unitWeight || undefined,
...
const editForm = reactive({ name: '', unitWeight: undefined as number | undefined, productType: 'EXPORT' as 'EXPORT' | 'DOMESTIC', imageUrl: undefined as string | undefined })

function startEdit() {
  if (!selectedItem.value) return
  editForm.name        = selectedItem.value.name
  editForm.unitWeight  = selectedItem.value.unitWeight ?? undefined
  editForm.productType = (selectedItem.value as any).productType ?? 'EXPORT'
...
async function saveEdit() {
  ...
  saving.value = true
  try {
    const updated = await productsStore.updateProduct(selectedItem.value.id, {
      name:        editForm.name.trim(),
      unitWeight:  editForm.unitWeight || undefined,
...
async function doSoftDelete() {
  if (!selectedItem.value) return
  try {
    await productsStore.softDeleteProduct(selectedItem.value.id)
...
async function doHardDelete() {
  if (!selectedItem.value) return
  try {
    await productsStore.hardDeleteProduct(selectedItem.value.id)
...
async function toggleShowDeleted() {
  ...
  if (showDeleted.value) {
    await productsStore.fetchDeletedProducts()
  }
}

async function doHardDeleteFromList(id: string) {
  try {
    await productsStore.hardDeleteProduct(id)
    await productsStore.fetchDeletedProducts()
    toastStore.showToast(L('ลบถาวรแล้ว', '已永久刪除'), 'success')
  } catch (e: any) {
    toastStore.showToast((e as any)?.response?.data?.message || L('ลบล้มเหลว', '刪除失敗'), 'danger')
  ...
  <div class="product-layout">
      ...
      <div v-if="showAddForm" class="add-wrap">
        <form class="add-form" @submit.prevent="saveAdd">
          <h2 class="form-title">{{ L('เพิ่มสินค้า', '新增商品') }}</h2>

          <div class="form-group">
            <label class="form-label">{{ L('รหัสสินค้า', '商品ID') }} <span class="required">*</span></label>
            <input v-model="addForm.id" class="form-input" :placeholder="L('ใส่รหัสสินค้า', '輸入商品ID')" required />
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            <label class="form-label">{{ L('น้ำหนักต่อหน่วย (kg)', '單件重量 (kg)') }}</label>
            <input v-model.number="addForm.unitWeight" type="number" step="0.001" min="0" class="form-input" :placeholder="L('เช่น 26.000', '例如 26.000')" />
          </div>
          <div class="form-group">
            <label class="form-label">{{ L('ประเภทสินค้า', '商品類型') }}</label>
            <div class="type-toggle-row">
              <button type="button" class="type-toggle-btn" :class="{ active: addForm.productType === 'EXPORT' }" @click="addForm.productType = 'EXPORT'">H02 {{ L('ส่งออก', '出口') }}</button>
              <button type="button" class="type-toggle-btn" :class="{ active: addForm.productType === 'DOMESTIC' }" @click="addForm.productType = 'DOMESTIC'">H03 {{ L('ภายใน', '境內') }}</button>
            </div>
          </div>
          <div class="form-group">
        ...
        <div class="list-panel">
          <div class="list-header">
            <span class="list-count">
              {{ showDeleted ? productsStore.deletedProducts.length : productsStore.products.length }} {{ L('รายการ', '筆') }}
            </span>
            <div class="header-actions">
              ...
              <button class="btn-add" @click="openAddForm">
                ...
                <span class="btn-add-text">{{ L('เพิ่มสินค้า', '新增商品') }}</span>
              </button>
            </div>
          </div>

          <template v-if="!showDeleted">
            <div v-if="productsStore.loading" class="list-state">
              <i class="pi pi-spin pi-spinner"></i>
            </div>
            <div v-else-if="productsStore.products.length === 0" class="list-state">
              <i class="pi pi-box"></i>
              <p>{{ L('ไม่มีข้อมูลสินค้า', '尚無商品資料') }}</p>
            </div>
          ...
          <template v-else>
            ...
            <div v-else-if="productsStore.deletedProducts.length === 0" class="list-state">
              <i class="pi pi-trash"></i>
              <p>{{ L('ไม่มีรายการที่ลบแล้ว', '沒有已刪除的記錄') }}</p>
            </div>
            <div v-else class="product-grid">
              <div
                v-for="item in productsStore.deletedProducts"
                :key="item.id"
                class="product-card deleted"
              >
        ...
        <div v-if="selectedItem" class="detail-drawer" :class="{ open: !!selectedItem }">
          ...
          <div class="drawer-body">
            <template v-if="isEditMode">
              <div class="form-group">
                <label class="form-label">{{ L('รหัสสินค้า', '商品ID') }}</label>
              ...
              <div class="form-group">
                <label class="form-label">{{ L('น้ำหนักต่อหน่วย (kg)', '單件重量 (kg)') }}</label>
                <input v-model.number="editForm.unitWeight" type="number" step="0.001" min="0" class="form-input" :placeholder="L('เช่น 26.000', '例如 26.000')" />
              </div>
              <div class="form-group">
                <label class="form-label">{{ L('ประเภทสินค้า', '商品類型') }}</label>
                <div class="type-toggle-row">
                  <button type="button" class="type-toggle-btn" :class="{ active: editForm.productType === 'EXPORT' }" @click="editForm.productType = 'EXPORT'">H02 {{ L('ส่งออก', '出口') }}</button>
                  <button type="button" class="type-toggle-btn" :class="{ active: editForm.productType === 'DOMESTIC' }" @click="editForm.productType = 'DOMESTIC'">H03 {{ L('ภายใน', '境內') }}</button>
                </div>
              </div>
              <div class="form-group">
                <label class="form-label">{{ L('รูปภาพ', '圖片') }}</label>
    ...
    <ConfirmDialog
      ...
      :message="L(`ต้องการลบสินค้า「${selectedItem?.name}」ใช่ไหม?`, `確定要刪除商品「${selectedItem?.name}」嗎？`)"
    ...
    <ConfirmDialog
      ...
      :title="L('ลบถาวร', '永久刪除')"
      :message="L(`การดำเนินการนี้จะลบสินค้า「${selectedItem?.name}」อย่างถาวรและไม่สามารถกู้คืนได้`, `此操作將永久刪除商品「${selectedItem?.name}」，且無法復原。確定繼續嗎？`)"
...
<style scoped>
.product-layout { height: 100%; display: flex; flex-direction: column; overflow: hidden; }
...
.product-main { flex: 1; display: flex; overflow: hidden; position: relative; }
...
.product-card { display: flex; align-items: flex-start; gap: 12px; padding: 12px; background: var(--color-card); border: 1px solid var(--color-border); border-radius: 10px; color: var(--color-text); text-align: left; width: 100%; transition: background 0.15s, border-color 0.15s; }
.product-card:hover { background: var(--color-card-hover); border-color: var(--color-text-dim); }
.product-card.active { border-color: var(--color-accent); background: var(--color-card-hover); }
...
.type-toggle-btn { flex: 1; padding: 10px; background: var(--color-card); border: 1px solid var(--color-border); border-radius: 8px; color: var(--color-text-muted); font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.15s; }
.type-toggle-btn.active { border-color: var(--color-accent); color: var(--color-accent); background: rgba(232,130,12,0.1); }
...
.product-card.deleted { opacity: 0.65; border-style: dashed; border-color: var(--color-danger); cursor: default; }
