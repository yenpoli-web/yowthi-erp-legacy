...
import { usePurchaseItemsStore } from '../../stores/purchase-items'
import { useAuthStore }          from '../../stores/auth'
import { useToastStore }         from '../../stores/toast'
import type { PurchaseItem } from '../../types/master-data'

const { locale } = useI18n()
const isThai = computed(() => locale.value === 'th')
function L(th: string, zh: string) { return isThai.value ? th : zh }

const purchaseItemsStore = usePurchaseItemsStore()
...
async function saveAdd() {
  ...
  saving.value = true
  try {
    await purchaseItemsStore.createPurchaseItem({
      id:       addForm.id.trim(),
...
function selectItem(item: PurchaseItem) {
...
async function saveEdit() {
  ...
  saving.value = true
  try {
    const updated = await purchaseItemsStore.updatePurchaseItem(selectedItem.value.id, {
...
async function doSoftDelete() {
  if (!selectedItem.value) return
  try {
    await purchaseItemsStore.softDeletePurchaseItem(selectedItem.value.id)
...
async function doHardDelete() {
  if (!selectedItem.value) return
  try {
    await purchaseItemsStore.hardDeletePurchaseItem(selectedItem.value.id)
...
async function toggleShowDeleted() {
  ...
  if (showDeleted.value) {
    await purchaseItemsStore.fetchDeletedPurchaseItems()
  }
}

async function doHardDeleteFromList(id: string) {
  try {
    await purchaseItemsStore.hardDeletePurchaseItem(id)
    await purchaseItemsStore.fetchDeletedPurchaseItems()
    toastStore.showToast(L('ลบถาวรแล้ว', '已永久刪除'), 'success')
  } catch (e: any) {
    toastStore.showToast((e as any)?.response?.data?.message || L('ลบล้มเหลว', '刪除失敗'), 'danger')
...
onMounted(() => purchaseItemsStore.fetchPurchaseItems())
</script>

<template>
  <div class="purchase-item-layout">

      <div v-if="showAddForm" class="add-wrap">
        <form class="add-form" @submit.prevent="saveAdd">
          <h2 class="form-title">{{ L('เพิ่มรายการ', '新增採購項目') }}</h2>

          <div class="form-group">
            <label class="form-label">{{ L('รหัส', '採購項目ID') }} <span class="required">*</span></label>
            <input v-model="addForm.id" class="form-input" :placeholder="L('ใส่รหัส', '輸入採購項目ID')" required />
          </div>
          <div class="form-group">
      ...
      <div v-else class="purchase-item-main">
        <div class="list-panel">
          <div class="list-header">
            <span class="list-count">
              {{ showDeleted ? purchaseItemsStore.deletedPurchaseItems.length : purchaseItemsStore.purchaseItems.length }} {{ L('รายการ', '筆') }}
            </span>
            <div class="header-actions">
              ...
              <button class="btn-add" @click="openAddForm">
                ...
                <span class="btn-add-text">{{ L('เพิ่มรายการ', '新增採購項目') }}</span>
              </button>
            </div>
          </div>

          <template v-if="!showDeleted">
            <div v-if="purchaseItemsStore.loading" class="list-state">
              <i class="pi pi-spin pi-spinner"></i>
            </div>
            <div v-else-if="purchaseItemsStore.purchaseItems.length === 0" class="list-state">
              <i class="pi pi-box"></i>
              <p>{{ L('ไม่มีข้อมูล', '尚無資料') }}</p>
            </div>
            <div v-else class="purchase-item-grid">
              <button
                v-for="item in purchaseItemsStore.purchaseItems"
                :key="item.id"
          ...
          <template v-else>
            ...
            <div v-else-if="purchaseItemsStore.deletedPurchaseItems.length === 0" class="list-state">
              <i class="pi pi-trash"></i>
              <p>{{ L('ไม่มีรายการที่ลบแล้ว', '沒有已刪除的記錄') }}</p>
            </div>
            <div v-else class="purchase-item-grid">
              <div
                v-for="item in purchaseItemsStore.deletedPurchaseItems"
                :key="item.id"
                class="purchase-item-card deleted"
              >
        ...
        <div v-if="selectedItem" class="detail-drawer" :class="{ open: !!selectedItem }">
          ...
          <div class="drawer-body">
            <template v-if="isEditMode">
              <div class="form-group">
                <label class="form-label">{{ L('รหัส', '採購項目ID') }}</label>
    ...
    <ConfirmDialog
      ...
      :message="L(`ต้องการลบรายการ「${selectedItem?.name}」ใช่ไหม?`, `確定要刪除採購項目「${selectedItem?.name}」嗎？`)"
    ...
    <ConfirmDialog
      ...
      :title="L('ลบถาวร', '永久刪除')"
      :message="L(`การดำเนินการนี้จะลบรายการ「${selectedItem?.name}」อย่างถาวรและไม่สามารถกู้คืนได้`, `此操作將永久刪除採購項目「${selectedItem?.name}」，且無法復原。確定繼續嗎？`)"
...
<style scoped>
.purchase-item-layout { height: 100%; display: flex; flex-direction: column; overflow: hidden; }
...
.purchase-item-main { flex: 1; display: flex; overflow: hidden; position: relative; }
...
.purchase-item-grid { display: grid; grid-template-columns: 1fr; gap: 8px; }
@media (min-width: 540px) { .purchase-item-grid { grid-template-columns: repeat(2, 1fr); } }
@media (min-width: 1280px) { .purchase-item-grid { grid-template-columns: repeat(3, 1fr); } }
.purchase-item-card { display: flex; align-items: flex-start; gap: 12px; padding: 12px; background: var(--color-card); border: 1px solid var(--color-border); border-radius: 10px; color: var(--color-text); text-align: left; width: 100%; transition: background 0.15s, border-color 0.15s; }
.purchase-item-card:hover { background: var(--color-card-hover); border-color: var(--color-text-dim); }
.purchase-item-card.active { border-color: var(--color-accent); background: var(--color-card-hover); }
...
.purchase-item-card.deleted { opacity: 0.65; border-style: dashed; border-color: var(--color-danger); cursor: default; }
