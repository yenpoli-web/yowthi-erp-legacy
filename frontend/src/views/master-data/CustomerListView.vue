...
const addForm = reactive({ id: '', name: '', phone: '', address: '', imageUrl: undefined as string | undefined })

function resetAddForm() {
  addForm.id = ''; addForm.name = ''; addForm.phone = ''; addForm.address = ''; addForm.imageUrl = undefined
}
...
const editForm = reactive({ name: '', phone: '', address: '', imageUrl: undefined as string | undefined })
...
async function saveEdit() {
  if (!selectedCustomer.value || !editForm.name.trim()) return
  saving.value = true
  try {
...
async function doSoftDelete() {
  if (!selectedCustomer.value) return
  try {
    await customersStore.softDeleteCustomer(selectedCustomer.value.id)
...
async function doHardDelete() {
  if (!selectedCustomer.value) return
  try {
    await customersStore.hardDeleteCustomer(selectedCustomer.value.id)
...
async function toggleShowDeleted() {
  ...
  if (showDeleted.value) {
    await customersStore.fetchDeletedCustomers()
  }
}

async function doHardDeleteFromList(id: string) {
  try {
    await customersStore.hardDeleteCustomer(id)
    await customersStore.fetchDeletedCustomers()
    toastStore.showToast(L('ลบถาวรแล้ว', '已永久刪除'), 'success')
  } catch (e: any) {
    toastStore.showToast((e as any)?.response?.data?.message || L('ลบล้มเหลว', '刪除失敗'), 'danger')
  ...
  <div class="customer-layout">
      ...
      <div v-if="showAddForm" class="add-wrap">
        <form class="add-form" @submit.prevent="saveAdd">
          <h2 class="form-title">{{ L('เพิ่มลูกค้า', '新增客戶') }}</h2>

          <div class="form-group">
            <label class="form-label">{{ L('รหัสลูกค้า', '客戶ID') }} <span class="required">*</span></label>
            <input v-model="addForm.id" class="form-input" :placeholder="L('ใส่รหัสลูกค้า', '輸入客戶ID')" required />
          </div>
          <div class="form-group">
      ...
      <div v-else class="customer-main">
        <div class="list-panel">
          <div class="list-header">
            <span class="list-count">
              {{ showDeleted ? customersStore.deletedCustomers.length : customersStore.customers.length }} {{ L('รายการ', '筆') }}
            </span>
            <div class="header-actions">
              ...
              <button class="btn-add" @click="openAddForm">
                ...
                <span class="btn-add-text">{{ L('เพิ่มลูกค้า', '新增客戶') }}</span>
              </button>
            </div>
          </div>

          <template v-if="!showDeleted">
            <div v-if="customersStore.loading" class="list-state">
              <i class="pi pi-spin pi-spinner"></i>
            </div>
            <div v-else-if="customersStore.customers.length === 0" class="list-state">
              <i class="pi pi-users"></i>
              <p>{{ L('ไม่มีข้อมูลลูกค้า', '尚無客戶資料') }}</p>
            </div>
            <div v-else class="customer-grid">
              ...
              >
                <span class="card-avatar" :style="customer.imageUrl ? {} : { background: avatarColor(customer.id) }">
                  <img v-if="customer.imageUrl" class="card-avatar--img" :src="resolveImageUrl(customer.imageUrl)" :alt="customer.name" />
                  <template v-else>{{ initials(customer.name) }}</template>
                </span>
                <div class="card-body">
                  <span class="card-id-badge">{{ customer.id }}</span>
                  <p class="card-name">{{ customer.name }}</p>
                  <p v-if="customer.phone" class="card-phone">{{ customer.phone }}</p>
                </div>
              </button>
            </div>
          </template>
          ...
          <template v-else>
            ...
            <div v-else-if="customersStore.deletedCustomers.length === 0" class="list-state">
              <i class="pi pi-trash"></i>
              <p>{{ L('ไม่มีรายการที่ลบแล้ว', '沒有已刪除的記錄') }}</p>
            </div>
            <div v-else class="customer-grid">
              <div
                v-for="customer in customersStore.deletedCustomers"
                :key="customer.id"
                class="customer-card deleted"
              >
                <span class="card-avatar" :style="customer.imageUrl ? { filter: 'grayscale(1)' } : { background: avatarColor(customer.id), filter: 'grayscale(1)' }">
                  <img v-if="customer.imageUrl" class="card-avatar--img" :src="resolveImageUrl(customer.imageUrl)" :alt="customer.name" />
                  <template v-else>{{ initials(customer.name) }}</template>
                </span>
                <div class="card-body">
                  ...
                  <p class="card-name">{{ customer.name }}</p>
                  <p v-if="customer.phone" class="card-phone">{{ customer.phone }}</p>
                  <p class="deleted-meta">🗑 {{ (customer as any).deletedAt ? new Date((customer as any).deletedAt).toLocaleDateString() : '' }}</p>
                  <button
                    v-if="authStore.role === 'ADMIN'"
                    class="btn btn-danger-solid btn-sm"
                    @click="doHardDeleteFromList(customer.id)"
        ...
        <div v-if="selectedCustomer" class="detail-drawer" :class="{ open: !!selectedCustomer }">
          <div class="drawer-header">
            <span
              class="drawer-avatar"
              :style="selectedCustomer.imageUrl ? {} : { background: avatarColor(selectedCustomer.id) }"
            >
              <img v-if="selectedCustomer.imageUrl" class="drawer-avatar--img" :src="resolveImageUrl(selectedCustomer.imageUrl)" :alt="selectedCustomer.name" />
              <template v-else>{{ initials(selectedCustomer.name) }}</template>
            </span>
            <div class="drawer-title-area">
              <span class="drawer-name">{{ selectedCustomer.name }}</span>
              <span class="drawer-id-badge">{{ selectedCustomer.id }}</span>
            </div>
          ...
          <div class="drawer-body">
            <template v-if="!isEditMode">
              <div class="field-row">
                ...
                <span class="field-value">{{ selectedCustomer.phone || '—' }}</span>
              </div>
              <div class="field-row">
                <span class="field-label">{{ L('ที่อยู่', '住址') }}</span>
                <span class="field-value">{{ selectedCustomer.address || '—' }}</span>
              </div>
            </template>
            <template v-else>
              <div class="form-group">
                <label class="form-label">{{ L('รหัสลูกค้า', '客戶ID') }}</label>
                <input :value="selectedCustomer.id" class="form-input" disabled />
              </div>
              <div class="form-group">
                <label class="form-label">{{ L('ชื่อ', '姓名') }} <span class="required">*</span></label>
    ...
    <ConfirmDialog
      ...
      :message="L(`ต้องการลบลูกค้า「${selectedCustomer?.name}」ใช่ไหม?`, `確定要刪除客戶「${selectedCustomer?.name}」嗎？`)"
    ...
    <ConfirmDialog
      ...
      :title="L('ลบถาวร', '永久刪除')"
      :message="L(`การดำเนินการนี้จะลบลูกค้า「${selectedCustomer?.name}」อย่างถาวรและไม่สามารถกู้คืนได้`, `此操作將永久刪除客戶「${selectedCustomer?.name}」，且無法復原。確定繼續嗎？`)"
...
<style scoped>
.customer-layout { height: 100%; display: flex; flex-direction: column; overflow: hidden; }
...
.customer-main { flex: 1; display: flex; overflow: hidden; position: relative; }
...
.customer-grid { display: grid; grid-template-columns: 1fr; gap: 8px; }
@media (min-width: 540px) { .customer-grid { grid-template-columns: repeat(2, 1fr); } }
@media (min-width: 1280px) { .customer-grid { grid-template-columns: repeat(3, 1fr); } }
.customer-card { display: flex; align-items: flex-start; gap: 12px; padding: 12px; background: var(--color-card); border: 1px solid var(--color-border); border-radius: 10px; color: var(--color-text); text-align: left; width: 100%; transition: background 0.15s, border-color 0.15s; }
.customer-card:hover { background: var(--color-card-hover); border-color: var(--color-text-dim); }
.customer-card.active { border-color: var(--color-accent); background: var(--color-card-hover); }
...
.customer-card.deleted { opacity: 0.65; border-style: dashed; border-color: var(--color-danger); cursor: default; }
